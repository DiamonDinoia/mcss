...
namespace Gpu {    
    ...
    */ 
    ...
    __global__ void compute_terminal_positions(curandState *states,
                    real_type *longHist, real_type *transHist,
                    real_type screening_param, real_type mean_free_path, 
                    real_type track_limit, const unsigned int longHistNumBins,
                    const unsigned int transHistNumBins, real_type longDistInvD,
                    real_type transDistInvD, int numSims) 
    {    
        ...
        unsigned int num_sims_per_thread = (unsigned int)ceil(float(numSims) / (blockDim.x*gridDim.x));
        ...
        for (int j = 0; j < num_sims_per_thread; j++) {
            ...
            // equally as possible
            local_idx = j*blockDim.x*gridDim.x + blockDim.x*blockIdx.x + threadX;
            ...
            if (local_idx < numSims) {        
                ...
                while (!stop) {
                    ...
                    if (trackLength > track_limit) { // Last step, so shorten it and stop afterwards
                ...
                unsigned int longIdx = (unsigned int) ((longitudinal_deviation + 1.0) * longDistInvD);
            
                // Compute transversal deviation and its bin index
                real_type transversal_deviation = sqrt(trackPosition_x * trackPosition_x 
                                        + trackPosition_y * trackPosition_y) / trackLength;
                unsigned int transIdx = (unsigned int) (transversal_deviation * transDistInvD);
    ...
    */ 
    ...
    Histograms Simulate(Material material, int numHists) {
        ...
        cuda_ret = cudaMalloc((void**)&states_d, (num_blocks*NUM_THREADS) * sizeof(curandState));
        ...
        cuda_ret = cudaMalloc((void**)&longHist_d, longiDistNumBin * sizeof(real_type));
        if (cuda_ret != cudaSuccess) {
            printf("ERROR: Failed to initialise longHist on the GPU.\n");
            exit(-1);
        }
        
        cuda_ret = cudaMalloc((void**)&transHist_d, transDistNumBin * sizeof(real_type));
        if (cuda_ret != cudaSuccess) {
        ...
        compute_terminal_positions<<<grid_dim, block_dim>>> (states_d, longHist_d, transHist_d,
                    theScrPar,theMFP, theLimit, longiDistNumBin, transDistNumBin, longiDistInvD,
                    transDistInvD, numHists);
        ...
        cuda_ret = cudaMemcpy(longHist_h, longHist_d, 
            longiDistNumBin * sizeof(real_type), cudaMemcpyDeviceToHost);
        if (cuda_ret != cudaSuccess) {
        ...
        memcpy(&(histograms.longiHist[0]), &(longHist_h[0]), longiDistNumBin*sizeof(real_type));
        memcpy(&(histograms.transHist[0]), &(transHist_h[0]), transDistNumBin*sizeof(real_type));
