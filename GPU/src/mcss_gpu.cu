...
__global__ static void compute_terminal_positions(
  ...
  for (int j = 0; j < thread_histories; j++) {
    ...
    bool stop             = false;
    do {
      real_type stepLength = -theMFP * std::log(curand_uniform(&local_state));
      ...
      if (!stop) {
        ...
        real_type u3         = cost;
        rotateToLabFrame(u1, u2, u3, aTrack.fDirection[0], aTrack.fDirection[1],
                         aTrack.fDirection[2]);
...
Histograms Simulate(Material material, int numHists) {
...
#ifndef USE_SHARED_MEMORY
  cuda_ret =
      cudaFuncSetCacheConfig(compute_terminal_positions, cudaFuncCachePreferL1);
#else
  ...
  cuda_ret = cudaMemset(longHist_d, 0, longiDistNumBin * sizeof(unsigned int));
  if (cuda_ret != cudaSuccess) {
    printf("ERROR: Failed to set longHist to zero on the GPU.\n");
    exit(-1);
  }

  cuda_ret = cudaMemset(transHist_d, 0, transDistNumBin * sizeof(unsigned int));
  if (cuda_ret != cudaSuccess) {
